...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        let stack_top = (buffer.as_ptr() as usize + STACK_SIZE -16) &!15;
        let mut ctx = TaskContext::default();
        ctx.ra = thread_wrapper as u64;
        ctx.sp = stack_top as u64;
        let new_green_thread = GreenThread{
            ctx,
            state : ThreadState::Ready,
            _stack : Some(buffer),
            entry : Some(entry)
        };
        self.threads.push(new_green_thread);
        //self.current += 1;
    ...
    fn schedule_next(&mut self) {
        ...
        let next_ctx_ptr = self.threads[next_idx].ctx.as_ptr();
