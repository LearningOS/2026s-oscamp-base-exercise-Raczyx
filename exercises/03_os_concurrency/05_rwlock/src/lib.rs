...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if (state&WRITER_HOLDING) != 0 || (state&WRITER_WAITING) != 0 || (state == READER_MASK) {
                core::hint::spin_loop();
                continue;
            }else {
                break ;
        ...
        self.state.compare_exchange(self.state.load(Ordering::Acquire),self.state.load(Ordering::Acquire)+1, Ordering::AcqRel, Ordering::Acquire);
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        /* 
        while self.state.load(Ordering::Acquire)&(!WRITER_WAITING) != 0 {
            core::hint::spin_loop();
        }*/
        while let Err(_) = self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire) {
            core::hint::spin_loop();
        }
