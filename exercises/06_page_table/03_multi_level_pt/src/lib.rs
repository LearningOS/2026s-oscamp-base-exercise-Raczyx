...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        //todo!()
        // let l2_vpn = self.extract_vpn(va,2);
        // let l1_vpn = self.extract_vpn(va,1);
        // let l0_vpn = self.extract_vpn(va,0);
        let vpn= [
            ((va >> 12) & 0x1FF) as usize,
            ((va >> 21) & 0x1FF) as usize,
            ((va >> 30) & 0x1FF) as usize
            ];
        let mut court_ppn = self.root_ppn;
        for i in 0..2 {
            let table = self.nodes.get(&court_ppn).unwrap();
            let pte = table.entries[vpn[2-i]];
            if pte & PTE_V == 0 {
                let n_ppn = self.alloc_node();
                let table = self.nodes.get_mut(&court_ppn).unwrap();
                table.entries[vpn[2-i]] = n_ppn << PPN_SHIFT | PTE_V;
                court_ppn = n_ppn ;
            }else {
                court_ppn = pte >> PPN_SHIFT;
            }
        }
        let table = self.nodes.get_mut(&court_ppn).unwrap();
        table.entries[vpn[0]] = (pa >> 12)<< PPN_SHIFT | (flags |PTE_V );
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for i in 0..2 {
            let table = self.nodes.get(&current_ppn).unwrap();
            let pte = table.entries[vpn[2 -i]];
            if pte & PTE_V == 0 {
            ...
            if pte & (PTE_R|PTE_W|PTE_X) != 0 {
                ...
                return TranslateResult::Ok(((pte >> 10) << 12) | (va &offset_mask));
            }
            current_ppn = pte >> 10;
        }
        let table = self.nodes.get(&current_ppn).unwrap();
        let pte = table.entries[vpn[0]];
        if pte & PTE_V == 0 {
            return TranslateResult::PageFault;
        }
        if pte & (PTE_R|PTE_W|PTE_X) != 0 {
            return TranslateResult::Ok(((pte >> 10) << 12) + offset);
        }else {
            return TranslateResult::PageFault;
        }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let mut current_ppn = self.root_ppn;
        let table = self.nodes.get(&current_ppn).unwrap();
        let l2_pte = table.entries[vpn[1]];
        if l2_pte & PTE_V == 0 {
            
            let n_ppn = self.alloc_node();
            let table = self.nodes.get_mut(&current_ppn).unwrap();
            table.entries[vpn[1]] = (n_ppn << 10) | PTE_V;
            current_ppn = n_ppn;
        }else {
            current_ppn = l2_pte >> 10;
        }
        let table = self.nodes.get_mut(&current_ppn).unwrap();
        table.entries[vpn[0]] = ((pa >> 12) << 10 ) | (flags | PTE_V);
