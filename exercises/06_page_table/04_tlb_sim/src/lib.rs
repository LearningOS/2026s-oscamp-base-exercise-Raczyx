...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        for i in &self.entries {
            if i.asid == asid && i.vpn == vpn && i.valid{
                self.stats.hits += 1;
                return Some(i.ppn);
            }
        }
    ...
    pub fn insert(&mut self, vpn: u64, ppn: u64, asid: u16, flags: u64) {
        ...
        for i in 0..self.capacity {
            let tlb = self.entries.get(i).unwrap();
            if tlb.valid && tlb.asid == asid && tlb.vpn == vpn {
                let tlb = self.entries.get_mut(i).unwrap();
                tlb.flags = flags;
                tlb.ppn = ppn;
                return;
            }
        ...
        let tlb = self.entries.get_mut(self.fifo_ptr).unwrap();
        tlb.asid = asid;
        tlb.flags = flags;
        tlb.ppn = ppn;
        tlb.valid = true;
    ...
    pub fn flush_all(&mut self) {
        ...
        self.entries.iter_mut().for_each(|i| i.valid = false);
    }
    ...
    pub fn valid_count(&self) -> usize {
        ...
        self.entries.iter().filter(|i| i.valid == true).count()
    }
}
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        for (asid,page) in &self.page_table{
            if *asid == self.current_asid && page.vpn == vpn {
                self.tlb.insert(vpn, page.ppn, self.current_asid, page.flags);
